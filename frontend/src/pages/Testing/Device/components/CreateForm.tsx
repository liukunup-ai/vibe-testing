...
import { createDevice } from '@/services/backend/device';
...
const CreateForm = ({ visible, onCancel, onSuccess }: CreateFormProps) => {
  ...
  const [form] = useForm<API.DeviceRequest>();
  const intl = useIntl();
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      await createDevice(values as API.DeviceRequest);
      message.success(
  ...
  return (
    <Modal
      title={
        <FormattedMessage
          id="pages.testing.device.modal.createForm.title"
          defaultMessage="新建设备"
        />
      }
      open={visible}
    ...
    >
      <Form form={form} layout="vertical" className="create-device-form">
        <Form.Item
          name="deviceNo"
          label={
