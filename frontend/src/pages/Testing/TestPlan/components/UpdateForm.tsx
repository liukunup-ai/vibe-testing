...
interface UpdateFormProps {
  ...
  onCancel: () => void;
  onSuccess: () => void;
  initialValues?: Partial<API.TestPlan>;
}
...
const UpdateForm = ({ visible, onCancel, onSuccess, initialValues }: UpdateFormProps) => {
  ...
  const [form] = useForm<API.TestPlan>();
  const intl = useIntl();
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      await updateTestPlan({ id: values.id }, submitValues as API.TestPlanRequest);
      message.success(
  ...
  return (
    <Modal
      title={
        <FormattedMessage
          id="pages.testing.testplan.modal.updateForm.title"
          defaultMessage="更新测试计划"
        />
      }
      open={visible}
      onOk={handleOk}
      ...
      width={700}
    >
      <Form form={form} layout="vertical" className="update-testplan-form">
