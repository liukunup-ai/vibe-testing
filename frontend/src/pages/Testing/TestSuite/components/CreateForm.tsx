...
import { createTestSuite } from '@/services/backend/testSuite';
...
const CreateForm = ({ visible, onCancel, onSuccess }: CreateFormProps) => {
  ...
  const [form] = useForm<API.TestSuiteRequest>();
  const intl = useIntl();
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      await createTestSuite(params as API.TestSuiteRequest);
      message.success(
  ...
  return (
    <Modal
      title={
        <FormattedMessage
          id="pages.testing.testsuite.modal.createForm.title"
          defaultMessage="新建测试套件"
        />
      }
      open={visible}
      onOk={handleOk}
      ...
      width={600}
    >
      <Form form={form} layout="vertical" className="create-testsuite-form">
        <Form.Item
          name="projectId"
          label={
            <FormattedMessage
