...
const Dashboard: React.FC = () => {
  ...
  const { data: projectData } = useRequest(() => listProjects({ page: 1, pageSize: 1 }));

  const { data: caseData } = useRequest(() =>
    listTestCases({ page: 1, pageSize: 1, projectId: 0 } as API.TestCaseSearchRequest),
  );

  const { data: deviceData } = useRequest(() => listDevices({ page: 1, pageSize: 100 }));
    ...
    >
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12} lg={6}>
          <StatisticCard
            statistic={{
              title: intl.formatMessage({
                id: 'pages.dashboard.projectCount',
                defaultMessage: '项目总数',
              }),
              value: projectCount,
              icon: <ProjectOutlined style={{ fontSize: 32, color: '#1890ff' }} />,
              suffix: intl.formatMessage({
                id: 'pages.dashboard.projectCount.suffix',
                defaultMessage: '个',
              }),
            }}
            style={{ height: '100%' }}
        ...
        <Col xs={24} sm={12} lg={6}>
          <StatisticCard
            statistic={{
              title: intl.formatMessage({
                id: 'pages.dashboard.caseCount',
                defaultMessage: '累计用例',
              }),
              value: caseCount,
              icon: <FileTextOutlined style={{ fontSize: 32, color: '#52c41a' }} />,
              suffix: intl.formatMessage({
                id: 'pages.dashboard.caseCount.suffix',
                defaultMessage: '条',
              }),
            }}
            style={{ height: '100%' }}
        ...
        <Col xs={24} sm={12} lg={6}>
          <StatisticCard
            statistic={{
              ...
              suffix: intl.formatMessage({
                id: 'pages.dashboard.deviceCount.suffix',
                defaultMessage: '台',
              }),
            }}
            style={{ height: '100%' }}
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col xs={24} lg={12}>
          <ProCard
            title={intl.formatMessage({
              id: 'pages.dashboard.passRateTrend',
        ...
        <Col xs={24} lg={12}>
          <ProCard
            title={intl.formatMessage({
              id: 'pages.dashboard.deviceUsage',
            ...
            <Row gutter={[16, 16]} style={{ padding: '20px 0' }}>
              <Col span={8}>
                <StatisticCard>
                  <Statistic
                    title={intl.formatMessage({
                      id: 'pages.dashboard.deviceOnline',
                      defaultMessage: '在线',
                    })}
                    value={onlineDevices}
                    suffix={`/ ${deviceCount}`}
              ...
              <Col span={8}>
                <StatisticCard>
                  <Statistic
                    title={intl.formatMessage({
                      id: 'pages.dashboard.deviceOffline',
                      defaultMessage: '离线',
                    })}
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col xs={24} lg={12}>
          <ProCard
            title={intl.formatMessage({
              id: 'pages.dashboard.recentTasks',
        ...
        <Col xs={24} lg={12}>
          <ProCard
            title={intl.formatMessage({
              id: 'pages.dashboard.pendingTasks',
