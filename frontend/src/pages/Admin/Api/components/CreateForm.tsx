...
const CreateForm = ({ visible, onCancel, onSuccess }: CreateFormProps) => {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      message.success(
        intl.formatMessage({ id: 'pages.common.object.new.success', defaultMessage: '新增成功' }),
      );
      form.resetFields();
      onSuccess();
    } catch (error) {
      const msg = intl.formatMessage({
        id: 'pages.common.object.new.error',
        defaultMessage: '新增失败',
      });
  ...
  return (
    <Modal
      title={
        <FormattedMessage id="pages.admin.api.modal.createForm.title" defaultMessage="新建接口" />
      }
      open={visible}
      onOk={handleOk}
      ...
      width={600}
    >
      <Form form={form} layout="vertical" className="create-api-form">
        <Form.Item
          name="name"
