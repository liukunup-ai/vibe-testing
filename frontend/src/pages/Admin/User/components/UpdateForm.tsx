...
const UpdateForm = ({ visible, onCancel, onSuccess, initialValues }: UpdateFormProps) => {
  ...
  return (
    <Modal
      title={
        <FormattedMessage id="pages.admin.user.modal.updateForm.title" defaultMessage="更新用户" />
      }
      open={visible}
      onOk={handleOk}
      ...
      width={600}
    >
      <Form form={form} layout="vertical" className="update-user-form">
        ...
        <Form.Item
          name="status"
          label={<FormattedMessage id="pages.admin.user.key.status" defaultMessage="状态" />}
          hidden
        >
          <Input disabled />
        </Form.Item>
        ...
        >
          <Select
            mode="multiple"
            placeholder={intl.formatMessage({
              id: 'pages.admin.user.form.roles.placeholder',
